...
const start = function start() {
        ...
        if (!config) {
          fetchConfig(
            ...
            (response) => {
              ...
              window.enabledEventSettings =
                response.enhancedConversionEventSettings;
            },
            "addPixels"
          );
        } else {
  ...
  function sendEventToFacebook(settings, event) {
    ...
    const { hubSpotFormId, eventCategory } = settings;
    const { conversionId: transaction_id, formGuid } = event; // https://developers.facebook.com/docs/marketing-api/conversions-api/deduplicate-pixel-and-server-events/
  ...
  function sendEventToGoogle(setting, event) {
    const { hubSpotFormId, pixelId, conversionLabel } = setting;
    const { conversionId: transaction_id, formGuid } = event;
