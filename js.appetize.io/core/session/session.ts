...
import { AllUI, TouchAction } from '../api/recorder/public';
...
> extends EventEmitter {
    ...
    async playAction<T extends Action | RecordedAction>(
        ...
        try {
            ...
            const mappedAction = noMap ? action : mapper.mapAction(action);

            const playActionPayload = {
                id,
                action: mappedAction,
                options: {
                    ...options,
            ...
            const [result] = await Promise.all([
                new Promise<PlayActionResult<any>>((resolve, reject) => {
                    const cleanup = () => {
                    ...
                    const handleSuccess = async (
                        ...
                        if (value.playback?.id !== id) return;
                        ...
                        switch (value.errorId) {
                            ...
                            case 'notFound': {
                                if (
                                    ...
                                    'element' in value.playback.action &&
                                    value.playback.action.element !== undefined
                                ) {
                                    reject(
                                        new ElementNotFoundError(
                                            value.playback.action.element
                                        )
                                    );
                                    break;
                                }
                            ...
                            case 'ambiguousMatch':
                                if (
                                    'matchedElements' in value &&
                                    value.matchedElements !== undefined
                                ) {
                                    reject(
                            ...
                            case 'invalidArgument': {
                                if ('element' in action && action.element) {
                                    if (
                                        ...
                                        reject(
                                            new ElementOutOfBoundsError(
                                                action as TouchAction
                                            )
                                        );
                                        break;
                                    }
                            ...
                            // fallthrough
                            default:
                                reject(new OperationalError(value.message));
                                break;
                        }
                    };

                    this.once('playbackFoundAndSent', handleSuccess);
                    this.once('playbackError', handleError);
                }),
                this.socket.send('playAction', playActionPayload),
            ]);
    ...
    async playActions<T extends Action | RecordedAction>(
        ...
        try {
            ...
            for (const action of actions) {
                ...
                // wait for 1s between each action unless we are playing back consecutive keypresses
    ...
    async swipe(
        ...
        try {
            ...
            const g = new SwipeGesture({ duration });
    ...
    async waitForAnimations(
        ...
        try {
            ...
            const timeoutId = setTimeout(() => {
                reject(
                    new TimeoutError(
                        `Timed out after ${timeout}ms waiting for animation to end. Waited for imageThreshold of ${imageThreshold} but lowest was ${lowestImageThreshold.toFixed(
                            2
                        )}`
                    )
                );
    ...
    protected actionLogs: ActionLog[] = [];

    // async because playwright session will use async methods
    protected async addActionLog(log: ActionLog) {
